...
/**
 * Mediator syscall tools — registers policy_propose, fork_with_policy,
 * ipc_send, etc. as native OpenClaw agent tools.
 *
...
/** Send a request to the mediator daemon over UDS. */
function callMediator(method, params) {
    return new Promise((resolve) => {
        ...
        const request = {
            id: `nemo-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`,
            method,
...
// ── Tool definitions ──────────────────────────────────────────────────
const MEDIATOR_TOOLS = [
    {
        name: "policy_propose",
        description: "Propose a new child policy for operator approval. This is how you acquire " +
            "capabilities you don't have (web access, file mounts, etc.). Blocks until " +
            "approved or denied (5-min timeout).\n\n" +
            "TRIFECTA RULE: never request a single policy that combines (1) sensitive " +
            "data mounts + (2) untrusted input + (3) external network egress. Split " +
            "into reader/processor/sender chains.",
        parameters: {
            type: "object",
            properties: {
                config: {
                    ...
                    properties: {
                        ...
                        allowed_ipc_targets: { type: "array", items: { type: "string" }, description: "Use ['init'] to talk back to parent" },
        ...
        execute: async (_id, params) => mediatorResult(await callMediator("policy_propose", params)),
    },
    {
        name: "fork_with_policy",
        description: "Fork a child workflow under an approved policy. Returns workflow_id, token, and UID.",
        parameters: {
            type: "object",
            properties: {
                workflow_id: { type: "string", description: "Unique ID for child workflow" },
                policy_name: { type: "string", description: "Approved policy to run under" },
                inherit: { type: "boolean", description: "Inherit parent capabilities (usually false)" },
            },
            required: ["workflow_id", "policy_name", "inherit"],
        },
        execute: async (_id, params) => mediatorResult(await callMediator("fork_with_policy", params)),
    },
    {
        name: "ipc_send",
        ...
        parameters: {
            type: "object",
            properties: {
                target_workflow_id: { type: "string", description: "Workflow to send to" },
                message: { description: "Arbitrary JSON payload" },
            },
            required: ["target_workflow_id", "message"],
        },
        execute: async (_id, params) => mediatorResult(await callMediator("ipc_send", params)),
    },
    {
        name: "mediator_ps",
    ...
    {
        name: "policy_get",
        description: "Get full details of an approved policy.",
        parameters: {
            type: "object",
            properties: { policy_name: { type: "string" } },
            required: ["policy_name"],
        },
        execute: async (_id, params) => mediatorResult(await callMediator("policy_get", params)),
    },
    {
        ...
        execute: async (_id, params) => mediatorResult(await callMediator("signal", params)),
    },
    {
        name: "request_port",
    ...
    },
    {
        ...
        execute: async (_id, params) => mediatorResult(await callMediator("revoke_policy", params)),
    },
];
// ── Registration ──────────────────────────────────────────────────────
export function registerMediatorTools(api) {
    // Only register if the mediator socket exists (skip on non-mediator sandboxes).
    const socket = process.env["MEDIATOR_SOCKET"] || DEFAULT_SOCKET;
    try {
        readFileSync(socket.replace(/\.sock$/, ".sock.token"), "utf-8");
    }
    catch {
        api.logger.debug("Mediator tools: token file not found — skipping tool registration " +
            "(mediator daemon may not be running)");
        return;
    }
    let registered = 0;
    for (const tool of MEDIATOR_TOOLS) {
        try {
            api.registerTool(tool);
            registered++;
        }
        catch (err) {
            api.logger.warn(`Mediator tools: failed to register '${tool.name}': ${err.message}`);
        }
    }
    if (registered > 0) {
        api.logger.info(`Mediator tools: registered ${registered} syscall tools`);
    }
}
//# sourceMappingURL=mediator-tools.js.map